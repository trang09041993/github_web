...
<title>Câu hỏi bài tập theo mã</title>
    ...
    <script src="Template/FrontendAdmin/js/code-jquery.js"></script>
    <script src="Template/FrontendAdmin/js/bootstrap.min.js"></script>
    	...
    	<script type="text/javascript">
					...
					function Xuatketqua()
					{
						...
						var url = "Lambtdoc?kq="+kq+"&num="+<%=request.getAttribute("numberpage")%>+"&readexeriseid="+<%=request.getAttribute("readexeriseid")%>;
				...
				<ul class="thumbnails">
					<li class="span12">
						<div class="thumbnail" >
							...
							<c:forEach items="${danhsachcauhoibtdoc}" var ="list">
								<p>
				...
				<div class="span12">
					...
					<div>
							...
							<c:if test="${numberpage == 1}">
							   ...
							   <a href = "Lambtdoc?pageid=${numberpage+1}&readexeriseid=<%=request.getAttribute("readexeriseid")%>" class="btn btn-info">Next</a>
							   
						   </c:if>
						   <c:if test="${numberpage == maxpageid}">
							   <a href = "Lambtdoc?pageid=${numberpage-1}&readexeriseid=<%=request.getAttribute("readexeriseid")%>" class="btn btn-info">Prev</a>
							   <input type="button" value="Đáp án" class="btn btn-info" onclick="Xuatketqua()"/>
							   <a href = "Lambtdoc?pageid=${numberpage}&readexeriseid=<%=request.getAttribute("readexeriseid")%>" class="btn btn-info">Làm lại</a>
